...
- **`gh` CLI writes only**: when you must perform a write through `gh`, mint the bot token with `ORFE_CALLER_NAME=Greg orfe auth token --repo throw-if-null/orfe` and pass it explicitly to `gh`.
