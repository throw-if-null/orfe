...
function matchesProjectByOwnerAndNumber(body, options) {
...
function matchesProjectAddItem(body, options) {
    return (isObject(body) &&
        typeof body.query === 'string' &&
...
function matchesProjectStatusLookup(body, options) {
...
function matchesProjectStatusFields(body, options) {
    return (isObject(body) &&
        typeof body.query === 'string' &&
...
function matchesProjectStatusUpdate(body, options) {
    return (isObject(body) &&
        typeof body.query === 'string' &&
...
export function createProjectLookupResponse(options) {
    return {
        data: {
            repositoryOwner: {
...
export function createProjectItemsConnection(nodes, options) {
    return {
        nodes,
        pageInfo: createPageInfo(options),
    };
}
export function createProjectFieldsConnection(nodes, options) {
    return {
        nodes,
        pageInfo: createPageInfo(options),
    };
}
export function createProjectStatusFieldNode(options) {
    return {
...
export function createProjectStatusValueNode(options) {
    return {
...
export function mockProjectGetStatusRequest(options) {
    ...
    return scope
        .post('/graphql', (body) => matchesProjectStatusLookup(body, {
        itemType: options.itemType,
        itemNumber: options.itemNumber,
        statusFieldName,
    }) && isObject(body) && isObject(body.variables) && body.variables.projectItemsCursor === (options.projectItemsCursor ?? null))
        .reply(options.graphqlStatus ?? 200, options.graphqlResponseBody ?? {
        data: {
            repository: options.itemType === 'issue'
                ? {
                    issue: {
...
export function mockProjectStatusFieldsRequest(options) {
    return nock('https://api.github.com')
        .post('/graphql', (body) => matchesProjectStatusFields(body, {
...
export function mockProjectStatusUpdateRequest(options) {
    return nock('https://api.github.com')
        .post('/graphql', (body) => matchesProjectStatusUpdate(body, {
        projectId: options.projectId ?? 'PVT_project_1',
...
export function mockProjectLookupRequest(options) {
    ...
    return scope
        .post('/graphql', (body) => matchesProjectByOwnerAndNumber(body, {
        projectOwner: options.projectOwner,
        projectNumber: options.projectNumber,
    }))
        .reply(options.graphqlStatus ?? 200, options.graphqlResponseBody ?? createProjectLookupResponse({ projectId: options.projectId ?? 'PVT_project_1' }));
}
export function mockProjectAddItemRequest(options) {
    ...
    return scope
        .post('/graphql', (body) => matchesProjectAddItem(body, {
        projectId: options.projectId ?? 'PVT_project_1',
...
//# sourceMappingURL=project-fixtures.js.map