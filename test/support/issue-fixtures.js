...
function matchesIssueStateLookup(body, issueNumber) {
...
function matchesMarkIssueAsDuplicate(body, duplicateId, canonicalId) {
    return (isObject(body) &&
        typeof body.query === 'string' &&
...
function matchesUnmarkIssueAsDuplicate(body, duplicateId, canonicalId) {
    return (isObject(body) &&
        typeof body.query === 'string' &&
...
export function createIssueRestResponse(issueNumber, overrides = {}) {
    return {
        number: issueNumber,
        title: 'Issue title',
        body: 'Issue body',
...
export function createIssueStateNode(options) {
    return {
        ...
        duplicateOf: options.duplicateOfIssueNumber !== undefined
            ? {
...
export function mockIssueGetRequest(options) {
    ...
    return nock('https://api.github.com')
        ...
        .reply(status, options.responseBody ?? {
        number: issueNumber,
...
export function mockIssueCreateRequest(options) {
    ...
    return nock('https://api.github.com')
        ...
        .post('/app/installations/42/access_tokens')
        .reply(201, { token: 'ghs_123', expires_at: '2026-04-06T12:00:00Z' })
        .post(`/repos/${owner}/${repo}/issues`, (body) => JSON.stringify(body) === JSON.stringify(options.requestBody))
        .reply(status, options.responseBody ?? {
        number: 21,
        ...
        state: 'open',
        state_reason: null,
        labels: (options.requestBody.labels ?? []).map((name) => ({ name })),
        assignees: (options.requestBody.assignees ?? []).map((login) => ({ login })),
...
export function mockIssueUpdateRequest(options) {
    ...
    return nock('https://api.github.com')
        ...
        .reply(201, { token: 'ghs_123', expires_at: '2026-04-06T12:00:00Z' })
        .get(`/repos/throw-if-null/orfe/issues/${issueNumber}`)
        .reply(issueGetStatus, options.issueGetResponseBody ?? {
        number: issueNumber,
        title: 'Updated title',
        body: 'Updated body',
        state: 'open',
        ...
        .patch(`/repos/throw-if-null/orfe/issues/${issueNumber}`, (body) => JSON.stringify(body) === JSON.stringify(options.requestBody))
        .reply(status, options.responseBody ?? {
        number: issueNumber,
        title: 'Updated title',
        body: 'Updated body',
...
export function mockIssueCommentRequest(options) {
    ...
    return nock('https://api.github.com')
        ...
        .post(`/repos/throw-if-null/orfe/issues/${issueNumber}/comments`, (body) => JSON.stringify(body) === JSON.stringify({ body: options.body }))
        .reply(status, options.responseBody ?? {
        id: 123456,
...
export function mockIssueSetStateRequest(options) {
    ...
    if (options.includeGraphql !== false) {
        scope.post('/graphql', (body) => matchesIssueStateLookup(body, options.issueNumber)).reply(200, {
            data: { repository: { issue: options.currentIssueState } },
        });
    }
    if (options.unmark) {
        scope
            .post('/graphql', (body) => matchesUnmarkIssueAsDuplicate(body, options.unmark.duplicateId, options.unmark.canonicalId))
            .reply(200, { data: { unmarkIssueAsDuplicate: { clientMutationId: null } } });
    }
    if (options.restUpdateBody) {
        scope
            .patch(`/repos/throw-if-null/orfe/issues/${options.issueNumber}`, (body) => JSON.stringify(body) === JSON.stringify(options.restUpdateBody))
            .reply(200, createIssueRestResponse(options.issueNumber, options.restUpdateBody))
            .post('/graphql', (body) => matchesIssueStateLookup(body, options.issueNumber))
...
export function mockIssueSetStateDuplicateRequest(options) {
    ...
    if (options.includeGraphql !== false) {
        scope
            ...
            .post('/graphql', (body) => matchesIssueStateLookup(body, options.duplicateOfIssueNumber))
    ...
    if (options.mark) {
        scope
            .post('/graphql', (body) => matchesMarkIssueAsDuplicate(body, options.mark.duplicateId, options.mark.canonicalId))
...
//# sourceMappingURL=issue-fixtures.js.map