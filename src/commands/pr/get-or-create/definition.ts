...
export const prGetOrCreateCommand = createCommandDefinition({
  ...
  usage:
    'orfe pr get-or-create --head <branch> --title <text> [--body <text>] [--template <name@version>] [--base <branch>] [--draft] [--repo <owner/name>] [--config <path>] [--auth-config <path>]',
  ...
  options: [
    ...
    { key: 'body', flag: '--body', description: 'Pull request body.', type: 'string' },
    {
      key: 'template',
      flag: '--template',
      description: 'Validate PR body against a versioned template and append provenance.',
      type: 'string',
    },
