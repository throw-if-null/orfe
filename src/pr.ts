...
interface PullRequestReplyData {
  pr_number: number;
  comment_id: number;
...
export async function handlePrReply(context: CommandContext): Promise<PullRequestReplyData> {
  const prNumber = context.input.pr_number as number;
  ...
  try {
    const { rest } = await context.getGitHubClient();
    await assertPrReplyTargetIsPullRequest(rest, context.repo.owner, context.repo.name, prNumber);
...
async function assertPrCommentTargetIsPullRequest(
  ...
  await assertPrTargetIsPullRequest(rest, owner, repo, prNumber, mapPullRequestCommentError);
}

async function assertPrReplyTargetIsPullRequest(
  rest: Awaited<ReturnType<CommandContext['getGitHubClient']>>['rest'],
  owner: string,
  repo: string,
  prNumber: number,
...
function normalizePullRequestReplyResponse(
  prNumber: number,
